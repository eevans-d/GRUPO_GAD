...
tarea_asignaciones = Table(
    'tarea_asignaciones',
    Base.metadata,
    Column('tarea_id', Integer, ForeignKey('gad.tareas.id', ondelete="CASCADE"), primary_key=True),
    Column('efectivo_id', Integer, ForeignKey('gad.efectivos.id', ondelete="CASCADE"), primary_key=True),
    schema='gad'
)
...
class Usuario(Base):
    ...
    nivel = Column(Enum(NivelAutenticacion), nullable=False, default=NivelAutenticacion.uno)
...
class Efectivo(Base):
    ...
    estado_disponibilidad = Column(Enum(EstadoDisponibilidad), nullable=False, default=EstadoDisponibilidad.activo)
    ...
    tareas = relationship(
        "Tarea",
        secondary=tarea_asignaciones,
        back_populates="asignados"
    )
...
class Tarea(Base):
    ...
    inicio_programado = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
