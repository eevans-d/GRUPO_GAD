...
from sqlalchemy import (
    String, Integer, DateTime, Numeric, Text, 
    ForeignKey, Table, Column, CheckConstraint
)
from sqlalchemy.orm import Mapped, mapped_column, relationship
from sqlalchemy.dialects.postgresql import UUID as sa_UUID, JSONB, ENUM, ARRAY
...
class Tarea(Base):
    ...
    tipo: Mapped[TaskType] = mapped_column(
        ENUM(TaskType, name="tipo_tarea", schema="gad"),
        nullable=False
    )
    ...
    fin_programado: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True)
    )
    inicio_real: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True)
    )
    fin_real: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True)
    )
    ...
    # Efectivos asignados
    efectivos_asignados: Mapped[List[int]] = mapped_column(
        ARRAY(Integer), 
        default=[]
    )
    
    # Métricas calculadas (STORED)
    duracion_real_horas: Mapped[Optional[float]] = mapped_column(
        Numeric,
        nullable=True
    )
    
    # Metadata y notas
    notas: Mapped[dict] = mapped_column(JSONB, default={})
    extra_data: Mapped[dict] = mapped_column(JSONB, default={})
    
    # Relaciones ORM
    delegado_usuario: Mapped["Usuario"] = relationship(
        "Usuario",
        foreign_keys=[delegado_usuario_id],
        back_populates="tareas_delegadas"
    )
    ...
    def duracion_estimada_horas(self) -> float:
        ...
        if self.inicio_programado and self.fin_programado:
            return (self.fin_programado - self.inicio_programado).total_seconds() / 3600.0
    ...
    def esta_activa(self) -> bool:
        ...
        return self.estado in [TaskStatus.PROGRAMMED, TaskStatus.IN_PROGRESS, TaskStatus.PAUSED]
