...
async def government_health_check() -> Dict[str, Any]:
    """
    Comprehensive health check for government services (GRUPO_GAD).
    Designed for citizen service monitoring and SLA compliance.
    """
    ...
    # 1. Database Health (Critical for citizen data)
    try:
        db_start = time.time()
        async for session in get_db_session():
            await session.execute(text("SELECT 1"))
    ...
    # 2. Redis/Pub-Sub Health (Optional, for WebSocket scaling)
    redis_status = "not_configured"
    redis_impact = "minimal"
    
    if hasattr(settings, 'REDIS_HOST') and settings.REDIS_HOST:
        try:
            # Redis check would go here if redis client was exposed
    ...
    health_data["checks"]["redis"] = {
        "status": redis_status,
        "websocket_scaling": "enabled" if redis_status == "healthy" else "limited_to_single_worker",
        "citizen_service_impact": redis_impact,
        "checked_at": datetime.utcnow().isoformat()
    }
    
    # 3. System Resources (Performance monitoring)
    try:
        ...
        health_data["checks"]["system_resources"] = {
            ...
            "warnings": warnings,
            "performance_adequate": resource_status == "healthy",
            "checked_at": datetime.utcnow().isoformat()
        }
        
        if resource_status != "healthy" and health_data["status"] == "healthy":
    ...
    # 4. Citizen Service Availability Assessment
    db_healthy = health_data["checks"]["database"]["status"] == "healthy"
    resources_ok = health_data["checks"]["system_resources"].get("status") != "unhealthy"
    
    if db_healthy and resources_ok:
        availability = "full"
        impact = "none"
    elif db_healthy:
    ...
    health_data["citizen_service_availability"] = {
        "status": availability,
        "impact_level": impact,
        "core_services_available": db_healthy,
        "estimated_recovery_time": "immediate" if impact == "none" else "< 15 minutes"
    }
    
    # 5. SLA Compliance
    uptime_seconds = time.time() - _app_start_time
    uptime_percentage = 99.9  # Placeholder - should be calculated from monitoring
    
    avg_response_time = health_data["checks"]["database"].get("response_time_ms", 0)
    
    health_data["sla_compliance"] = {
        "uptime_percentage": uptime_percentage,
        "average_response_time_ms": round(avg_response_time, 2),
        "sla_target_uptime": 99.5,
        "sla_target_response_time": 500,
        "meeting_uptime_sla": uptime_percentage >= 99.5,
        "meeting_response_sla": avg_response_time <= 500,
        "overall_sla_compliance": uptime_percentage >= 99.5 and avg_response_time <= 500
    }
    
    # Total check time
    health_data["total_check_time_ms"] = round((time.time() - check_start) * 1000, 2)
