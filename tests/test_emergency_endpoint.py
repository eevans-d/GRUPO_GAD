...
def test_emergency_endpoint_validation_error_invalid_lat(client, token_factory, mock_user):
    ...
    with patch("src.core.database.get_db_session", return_value=MagicMock()):
        with patch("src.api.dependencies.get_current_active_user", return_value=mock_user):
            response = client.post(
                "/api/v1/tasks/emergency",
                json={
                    "telegram_id": 123456789,
    ...
    assert any("lat" in str(error).lower() for error in error_detail)
...
def test_emergency_endpoint_validation_error_invalid_lng(client, token_factory, mock_user):
    ...
    assert response.status_code == 422
    error_detail = response.json()["detail"]
    assert any("lng" in str(error).lower() for error in error_detail)
...
def test_emergency_endpoint_no_efectivos_available(client, token_factory, mock_user):
    ...
    with patch("src.core.database.get_db_session", return_value=AsyncMock()):
        with patch("src.api.dependencies.get_current_active_user", return_value=mock_user):
            with patch("src.core.geo.postgis_service.find_nearest_efectivo", return_value=[]):
                with patch("src.api.utils.logging.log_business_event") as mock_log:
                    response = client.post(
                        "/api/v1/tasks/emergency",
                        json={
                            "telegram_id": 123456789,
...
def test_emergency_endpoint_success_with_postgresql(client, token_factory, mock_user):
    ...
    with patch("src.core.database.get_db_session", return_value=AsyncMock()):
        with patch("src.api.dependencies.get_current_active_user", return_value=mock_user):
            with patch("src.core.geo.postgis_service.find_nearest_efectivo") as mock_find_nearest:
                with patch("src.api.utils.logging.log_business_event") as mock_log:
